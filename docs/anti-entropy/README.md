...
| D40 | Gli scenari che il tester **salva** dal cruscotto si versionano **con la loro glue**: feature in `src/features/<app>/<flusso>/`, step in `src/steps/<app>/<flusso>/`, Page Object in `src/pages/<app>/`, condivise per applicazione. Una Page Object gia' salvata si riusa o **cresce** con i metodi che mancano, e non ne perde mai uno; una modificata a mano non si tocca. Una frase gia' definita altrove ferma il salvataggio. Si pianifica tutto prima di scrivere. Il passo di verifica generico vive una volta sola in `src/steps/common/` | Decisione del proprietario (2026-09-24): il repository e' privato, e il framework di test UI deve costruirsi da solo a ogni registrazione. Ribalta la regola del 16/09 (codice generato fuori da git) solo per cio' che il tester sceglie di salvare; il resto di `*/generated/` e `reports/` resta fuori. Uno scenario versionato senza i suoi step sarebbe stato un test che non parte su nessun'altra macchina |
