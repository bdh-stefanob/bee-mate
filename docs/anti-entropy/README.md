...
| F23 | `kiro-cli agent list` vede entrambi gli agenti dalla cartella `.kiro/agents` del workspace. **Ma l'agente "di sola lettura" ha eseguito una shell e creato un file** (2026-09-16): i nomi degli strumenti nella copia Kiro erano tradotti (`read`, `write`, `shell`), l'engine non li ha riconosciuti e ha concesso i suoi default. La scrittura e' fallita solo perche' il nome tradotto era un segnaposto vuoto | La garanzia su cui si regge D29 non era vera, e sarebbe stata detta in presentazione. Un limite dichiarato e non applicato e' peggio di nessun limite, perche' ci si conta sopra. Con un nome che non conosce, l'engine **non nega: concede** |
...
| D29 | Gli **agenti** si generano come le regole: sorgente `.amazonq/cli-agents/`, copia `.kiro/agents/`. La mappa degli strumenti e' esplicita e **fallisce** su un nome sconosciuto | Un agente conta piu' di una regola: una regola orienta, un agente **impedisce**. `bdd-authoring` con i soli strumenti di lettura non puo' modificare un file nemmeno volendo. Una traduzione sbagliata degli strumenti gli restituirebbe il permesso di scrivere senza che niente lo segnali. **CORRETTO il 2026-09-16**: la mappa di traduzione era essa stessa il difetto che prevedeva (F23). Gli strumenti non si traducono: i nomi validi stanno in `scripts/lib/kiro-tools.ts`, e `check:kiro-tools` verifica i file generati, non le intenzioni |
