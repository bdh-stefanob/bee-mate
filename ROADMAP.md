...
## 9. Aggiunte multi-app + step status (decisioni 2026-06-09)

Decisioni architetturali assunte (vedi `PROJECT_BRIEF.md` §4 e `WORKFLOW.md`):

- Multi-app con placeholder `app-a`, `app-b` (vedi `DOMAINS.md`)
- Step lifecycle: `implemented` / `wanted` / `deprecated` tracciati via JSDoc tags
- Collaborazione QA manuale ↔ SDET formalizzata in `WORKFLOW.md`

Task tecnici che derivano:

### 9.1 Refactor scaffold mono-app → multi-app  [PRIORITA 1, 1 giorno]

Spostare i contenuti attuali sotto `app-a/`:
...
- `src/pages/*.page.ts` → `src/pages/app-a/`
- `src/actions/*.actions.ts` → `src/actions/app-a/`

Aggiornare:
- `cucumber.js` (glob paths)
- `tsconfig.json` (paths/aliases se servono)
- Import statements nei file
- README e CONTRIBUTING (gia parziale)

Test: `npm test`, `npm run catalog`, conferma che 5 scenari/18 step continuano
a passare con nuovi path.

### 9.2 Estendere `extract-steps.ts` per app/area/status  [PRIORITA 1, 0.5 giorni]

Modifiche:
- Regex domain: `steps[\\/]([^\\/]+)(?:[\\/]([^\\/]+))?[\\/]` → produce `app` + `area?`
- Catalog schema: aggiungere `app: string`, `area?: string`, mantenendo
  `domain` come `${app}/${area}` per retro-compat
- Parsing JSDoc: gestire `@wanted`, `@deprecated`, `@replacedBy`, `@requester`,
  `@assignee`, `@page`
- Catalog step schema: aggiungere `status: 'implemented' | 'wanted' | 'deprecated'`,
  `replacedBy?: string`, `requester?: string`, `assignee?: string`, `page?: string`

Test: aggiornare il `STEP_CATALOG.md` di esempio (deve includere `@wanted`
demo step) e verificare che il render markdown non rompa.

### 9.3 Aggiornare i type dell'extension  [PRIORITA 1, 0.25 giorni]

In `vscode-extension/src/catalog/types.ts`:
- Aggiungere `status`, `replacedBy?`, `requester?`, `assignee?`, `app?`, `area?`
  a `CatalogStep` (`page?` gia presente)

Nel `CompletionItemProvider`:
- Suggerimenti `wanted` mostrati con `CompletionItemKind.Snippet` ma con
  warning nella description (es. `"BDD Catalog · ${domain} · WANTED"`)
- Suggerimenti `deprecated` ordinati per ultimi e con strikethrough nel label
  se possibile, e `@replacedBy` evidenziato nella documentation

### 9.4 Quickfix "Request step implementation"  [PRIORITA 2, 1 giorno]

Aggiunge al DiagnosticProvider un CodeAction quando uno step non e' nel catalog:
- "Request step implementation" → apre form (vscode.window.showInputBox) per
  app/area, requester (default = git config user.email), assignee opzionale
- Genera lo stub in `src/steps/<app>/<area>/<area>.steps.ts` con `@wanted`
- Apre la PR via `gh` CLI con label `step-wanted`

### 9.5 CI: rigenera + committa il catalog  [PRIORITA 1, 0.5 giorni]

GitHub Action su push a `main`:
1. `npm ci`
2. `npm run catalog`
3. Se `step-catalog.json` o `STEP_CATALOG.md` sono cambiati: `git commit` +
   `git push` (con `[skip ci]` per evitare loop) oppure apri PR di
   sincronizzazione
4. Pre-commit hook locale: blocca commit se il catalog locale e' out-of-sync
   col codice (richiede di lanciare `npm run catalog`)
