...
describe('getFileSHA', () => {
  ...
  it('returns sha string on 200', async () => {
    global.fetch = mockFetch([{ status: 200, body: { sha: 'abc123' } }]) as unknown as typeof fetch;
    const result = await getFileSHA('owner', 'repo', 'file.json', 'main', 'tok');
    expect(result).toBe('abc123');
  });

  it('returns undefined on 404', async () => {
    global.fetch = mockFetch([{ status: 404, body: { message: 'Not Found' } }]) as unknown as typeof fetch;
    const result = await getFileSHA('owner', 'repo', 'file.json', 'main', 'tok');
    expect(result).toBeUndefined();
  });

  it('throws on other status codes', async () => {
    global.fetch = mockFetch([{ status: 500, body: {} }]) as unknown as typeof fetch;
...
describe('getFileContent', () => {
  ...
  it('returns decoded content and sha on 200', async () => {
    ...
    global.fetch = mockFetch([{ status: 200, body: { content: raw, sha: 'sha-xyz' } }]) as unknown as typeof fetch;
  ...
  it('returns undefined on 404', async () => {
    global.fetch = mockFetch([{ status: 404, body: {} }]) as unknown as typeof fetch;
    const result = await getFileContent('owner', 'repo', 'missing.json', 'catalog', 'tok');
    expect(result).toBeUndefined();
  });

  it('throws on other error status', async () => {
    global.fetch = mockFetch([{ status: 403, body: {} }]) as unknown as typeof fetch;
...
describe('putContents', () => {
  ...
  it('throws with GitHub error message on non-ok response', async () => {
    global.fetch = mockFetch([{ status: 422, body: { message: 'Validation Failed' } }]) as unknown as typeof fetch;
...
describe('branchEnsure', () => {
  ...
  it('throws when branch GET returns unexpected status', async () => {
    global.fetch = mockFetch([{ status: 403, body: { message: 'Forbidden' } }]) as unknown as typeof fetch;
